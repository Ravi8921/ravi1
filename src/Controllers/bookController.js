...
let uploadFile = async (file) => {
    return new Promise(function (resolve, reject) { // exactly 
        ...
        var uploadParams = {
            ...
            Key: "ravi_bookcover-" + file.originalname, // HERE    "pk_newFolder/harry-potter.png" pk_newFolder/harry-potter.png
            Body: file.buffer,
        };
...
const createBook = async function (req, res) {
    try {
        ...
        const { title, excerpt, userId, ISBN, category, subcategory, releasedAt, image } = req.body
        ...
        if (!isValid(releasedAt)) {
            ...
            if (release[0].length == 4 && release[1].length == 2 && release[2].length == 2) {
                if ((validateDate(releasedAt, responseType = "boolean") == true)) {
                    let savedbook = await bookModel.create({ title, excerpt, userId, ISBN, category, subcategory, releasedAt,image });
...
const getAllBooks = async function (req, res) {
        try {
            ...
            if (Array.isArray(books) && books.length === 0) {   //this sitution is for when the user gives details but details does not matches with any of the books.
                return res.status(404).send({ status: false, message: 'No book found' })
            }

            query.isDeleted = false      //for finding only those books which are not deleted and they are publish.
            let allbooks = await bookModel.find(query).sort({ title: 1 }).select({ _id: 1, title: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1, reviews: 1 })
            //this time this sitution is for when we found that books given details are valid now we will show only those data which are not deleted and are publish with query in it.
    ...
    module.exports.coverBook = coverBook