...
const londonTemp = async function (req, res) {
  try {
    ...
    let keys = req.query.id

    let options = {
      method: "get",
      url: `http://api.openweathermap.org/data/2.5/weather?q=${city}&id=${keys}`,
    };
...
try{
      let x = ["Mumbai", "Kolkata","Delhi","Chennai", "London","Bengaluru", "Moscow"]
      let cityTemp = []
      for (let i = 0; i < x.length; i++) {
        obj = { city: x[i] }
        let response = await axios.get(`http://api.openweathermap.org/data/2.5/weather?q=${x[i]}&appid=45bd408d5c5c7cb51f0e7c1b445d1991`);
...
const london = async function (req, res) {
  try {
    ...
    let options = {
      method: "get",
      url: `http://localhost:3000/city/temp?q=London&appid=45bd408d5c5c7cb51f0e7c1b445d1991`,
    };
    const weatherdata = await axios(options);
